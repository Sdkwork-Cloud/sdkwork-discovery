use sdkwork_discovery_storage_postgres::sql;
...
fn query_sql_filters_soft_deleted_and_expired_records() {
    assert!(sql::DISCOVER_INSTANCES_PREFIX.contains("deleted_at IS NULL"));
    assert!(sql::DISCOVER_INSTANCES_PREFIX.contains("expires_at_ms >"));
    assert!(sql::RETRIEVE_INSTANCE.contains("instance_id = $4"));
    assert!(sql::RETRIEVE_INSTANCE.contains("deleted_at IS NULL"));
    assert!(sql::RETRIEVE_INSTANCE.contains("expires_at_ms > $5"));
...
fn config_sql_uses_immutable_release_rows() {
    ...
    assert!(sql::MARK_DRAFT_PUBLISHED.contains("published = true"));
