...
#[async_trait]
impl RegistryStore for SqliteDiscoveryStore {
    ...
    async fn renew_lease(
        ...
        let lease_scope = sqlx::query(
            "SELECT namespace, environment FROM discovery_service_instance \
             WHERE lease_id = ? AND deleted_at IS NULL AND expires_at_ms > ?",
        )
    ...
    async fn discover_instances(
        ...
        let rows = sqlx::query(sql::DISCOVER_INSTANCES)
            ...
            .bind(query.protocol.as_deref())
            .bind(if query.healthy_only { 1_i64 } else { 0_i64 })
            .fetch_all(self.pool())
            .await
            .map_err(sqlx_error)?;
