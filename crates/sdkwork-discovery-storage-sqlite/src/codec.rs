...
use crate::validation::{i64_to_u64, u32_to_i64};
...
pub(crate) fn decode_instance_status(
    ...
    match status {
        ...
        _ => Err(DiscoveryError::InvalidConfig(format!(
            "sqlite stored unknown instance status: {status}"
        ))),
    }
}
...
pub(crate) fn decode_config_format(format: &str) -> DiscoveryResult<ConfigFormat> {
    match format {
        ...
        _ => Err(DiscoveryError::InvalidConfig(format!(
            "sqlite stored unknown config format: {format}"
        ))),
    }
}
...
pub(crate) fn decode_config_scope(
    ...
    match kind {
        ...
        "application" => Ok(ConfigScope::Application {
            application: application.ok_or_else(|| {
                DiscoveryError::InvalidConfig(
                    "sqlite application config scope is missing application".to_string(),
                )
            })?,
        }),
        "service" => Ok(ConfigScope::Service {
            application: application.ok_or_else(|| {
                DiscoveryError::InvalidConfig(
                    "sqlite service config scope is missing application".to_string(),
                )
            })?,
            service_name: service_name.ok_or_else(|| {
                DiscoveryError::InvalidConfig(
                    "sqlite service config scope is missing service_name".to_string(),
                )
            })?,
        }),
        _ => Err(DiscoveryError::InvalidConfig(format!(
            "sqlite stored unknown config scope: {kind}"
        ))),
    }
}
...
pub(crate) fn decode_event_kind(kind: &str) -> DiscoveryResult<DiscoveryEventKind> {
    match kind {
        ...
        _ => Err(DiscoveryError::InvalidConfig(format!(
            "sqlite stored unknown event kind: {kind}"
        ))),
    }
}
...
pub(crate) fn service_instance_from_row(row: &SqliteRow) -> DiscoveryResult<ServiceInstance> {
    ...
    let weight: i64 = row.try_get("weight").map_err(sqlx_error)?;
    let priority: i64 = row.try_get("priority").map_err(sqlx_error)?;
    ...
    Ok(ServiceInstance {
        ...
        weight: u32::try_from(weight).map_err(|_| {
            DiscoveryError::InvalidConfig("sqlite returned invalid weight".to_string())
        })?,
        priority: u32::try_from(priority).map_err(|_| {
            DiscoveryError::InvalidConfig("sqlite returned invalid priority".to_string())
        })?,
...
pub(crate) fn draft_from_row(row: &SqliteRow) -> DiscoveryResult<ConfigDraft> {
    ...
    let published: i64 = row.try_get("published").map_err(sqlx_error)?;
...
pub(crate) fn release_from_row(row: &SqliteRow) -> DiscoveryResult<ConfigRelease> {
    let format: String = row.try_get("config_format").map_err(sqlx_error)?;
    let scope_kind: String = row.try_get("scope_kind").map_err(sqlx_error)?;
    let scope_application: Option<String> = row.try_get("scope_application").map_err(sqlx_error)?;
    let scope_service_name: Option<String> =
...
pub(crate) fn event_from_row(row: &SqliteRow) -> DiscoveryResult<DiscoveryEvent> {
    let revision: i64 = row.try_get("revision").map_err(sqlx_error)?;
...
pub(crate) fn bind_weight(value: u32) -> DiscoveryResult<i64> {
    u32_to_i64("weight", value)
}

pub(crate) fn bind_priority(value: u32) -> DiscoveryResult<i64> {
    u32_to_i64("priority", value)
}
