...
pub(crate) fn to_i64(field: &str, value: u64) -> DiscoveryResult<i64> {
    i64::try_from(value).map_err(|_| {
        DiscoveryError::InvalidArgument(format!("{field} is too large for sqlite INTEGER"))
    })
}
...
pub(crate) fn i64_to_u64(field: &str, value: i64) -> DiscoveryResult<u64> {
    u64::try_from(value)
        .map_err(|_| DiscoveryError::InvalidConfig(format!("sqlite returned negative {field}")))
}

pub(crate) fn u32_to_i64(field: &str, value: u32) -> DiscoveryResult<i64> {
    let _ = field;
    Ok(i64::from(value))
}
...
pub(crate) fn i64_to_usize(field: &str, value: i64) -> DiscoveryResult<usize> {
    usize::try_from(value)
        .map_err(|_| DiscoveryError::InvalidConfig(format!("sqlite returned invalid {field}")))
}
