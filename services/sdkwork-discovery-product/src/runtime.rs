...
use crate::bootstrap::{DiscoveryProductBootstrap, DiscoveryProductGrpcServer};

pub struct DiscoveryProductRuntime {
    bootstrap: DiscoveryProductBootstrap,
}
...
impl DiscoveryProductRuntime {
    ...
    pub fn from_toml_str_with_env(
        ...
        let bootstrap =
            DiscoveryProductBootstrap::from_toml_str_with_env(input, &options.env_overlay)?;
        Ok(Self { bootstrap })
    }
    ...
    pub fn from_process_env() -> DiscoveryResult<Self> {
        ...
        let bootstrap =
            DiscoveryProductBootstrap::from_toml_str_with_env(&config, &options.env_overlay)?;
        Ok(Self { bootstrap })
    }
    ...
    pub fn bootstrap(&self) -> &DiscoveryProductBootstrap {
        &self.bootstrap
    }

    pub fn bootstrap_mut(&mut self) -> &mut DiscoveryProductBootstrap {
        &mut self.bootstrap
    }
    ...
    pub async fn serve_grpc(mut self) -> DiscoveryResult<DiscoveryProductGrpcServer> {
